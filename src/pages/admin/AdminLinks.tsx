...
export default function AdminLinks() {
  ...
  async function handleCreate(e: React.FormEvent) {
    ...
    if (error) { toast.error('Erro ao criar link: ' + error.message); setSaving(false); return; }
  ...
  async function handleCreateSubLink(e: React.FormEvent) {
    ...
    if (error) { toast.error('Erro ao criar link: ' + error.message); setSubSaving(false); return; }
  ...
  function copyLink(token: string, prefix: string = 'catalogo') {
  ...
  function shareWhatsApp(link: AccessLink) {
    ...
    const message = encodeURIComponent(
      `Olá ${link.investor_name}! Aqui está seu acesso exclusivo ao catálogo de oportunidades da JMob: ${url}`
    );
        ...
        <DialogContent>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            <div className="space-y-2">
              ...
              <Input id="investor_name" value={form.investor_name} onChange={(e) => setForm(prev => ({ ...prev, investor_name: e.target.value }))} placeholder="João Silva" required />
            </div>
            <div className="space-y-2">
              <Label htmlFor="investor_email">Email</Label>
              <Input id="investor_email" type="email" value={form.investor_email} onChange={(e) => setForm(prev => ({ ...prev, investor_email: e.target.value }))} placeholder="joao@email.com" />
            </div>
            <div className="space-y-2">
              <Label htmlFor="investor_phone">WhatsApp</Label>
              <Input id="investor_phone" value={form.investor_phone} onChange={(e) => setForm(prev => ({ ...prev, investor_phone: e.target.value }))} placeholder="5511999999999" />
            </div>
            <div className="space-y-2">
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={saving}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Gerar Link</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
      ...
      <Dialog open={subDialogOpen} onOpenChange={setSubDialogOpen}>
        <DialogContent>
          ...
          <form onSubmit={handleCreateSubLink} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="sub_label">Descrição do Link</Label>
              <Input id="sub_label" value={subForm.label} onChange={(e) => setSubForm({ label: e.target.value })} placeholder="Ex: Corretores parceiros, Imobiliária XYZ..." />
            </div>
            ...
            <DialogFooter>
              ...
              <Button type="submit" disabled={subSaving}>{subSaving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}Gerar Link</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
          ...
          <form onSubmit={handleCreate} className="space-y-4">
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setForm(prev => ({ ...prev, investor_email: e.target.value }))}
            ...
            <div className="space-y-2">
              ...
              <Input
                ...
                onChange={(e) => setForm(prev => ({ ...prev, investor_phone: e.target.value }))}
